...
PATH_TO_CKPT = TRAINED_MODEL_DIR + '/frozen_inference_graph_RGBD.pb'
# List of the strings that is used to add correct label for each box.
PATH_TO_LABELS = TRAINED_MODEL_DIR + '/hand_label_map_RGBD.pbtxt'
NUM_CLASSES = 1
...
def load_inference_graph():
    ...
    print("> ====== Loading RGB-D Hand Detection frozen graph into memory")
    detection_graph = tf.Graph()
    ...
    print(">  ====== RGB-D Hand Detection Inference graph loaded.")
...
def detection_results(detection_graph,frame,sess,score_threshold=0.2,crop_size = 200):
    global score_thresh
    score_thresh = score_threshold
    boxes, scores, classes = detect_objects(frame, detection_graph, sess)
    frame_with_infos,cropped,limits = draw_box_on_image(num_hands_detect, score_thresh, scores, boxes, classes, im_width, im_height, frame,crop_size = crop_size)

    return frame_with_infos,cropped[0],limits[0] 
...
def draw_box_on_image(num_hands_detect, score_thresh, scores, boxes, classes,
 im_width, im_height, image_np,crop_size=200):
    color = None
    color0 = (0,0,255)
    color1 = (0,50,255)
    cropped_hand = np.zeros((crop_size,crop_size,3))
    [xmin,xmax,ymin,ymax]=[0,im_height,0,im_width]
    list_of_bounds = []
    cropped_hands =[]
    for i in range(num_hands_detect):
        if (scores[i] > score_thresh):
            if classes[i] == 1: id = 'hand'
            if i == 0: color = color0
            else: color = color1
           ...
           # print("left :" ,left,"   right: ",right,"    top : " ,top,"  bottom: " ,bottom)
            ...
            top = top + (bottom-top)/2   -  crop_size/2
            bottom = top + crop_size
            left = left + (right-left)/2   -  crop_size/2
        ...
        list_of_bounds.append([xmin,xmax,ymin,ymax])
        cropped_hands.append(cropped_hand)
    return image_np,cropped_hands,list_of_bounds 
