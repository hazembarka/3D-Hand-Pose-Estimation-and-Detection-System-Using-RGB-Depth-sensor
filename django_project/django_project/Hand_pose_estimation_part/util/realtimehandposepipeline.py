...
from django_project.Hand_pose_estimation_part.util.handdetector import HandDetector
...
class RealtimeHandposePipeline(object):
    ...
    def __init__(self, poseNet, config, di, verbose=False, comrefNet=None,maxDepth=2000,minDepth=20):
        """
        Initialize data
    ...
    def detect(self, frame,loc_rgbd_detector ):
        """
        Detect the hand
        ...
        hd = HandDetector(frame, self.sync['config']['fx'], self.sync['config']['fy'], importer=self.importer, refineNet=self.comrefNet,maxDepth=self.maxDepth,minDepth=self.minDepth)
        ...
        if numpy.allclose(loc, 0):
            return numpy.zeros(self.sync['config']['im_size'], dtype='float32'), numpy.eye(3), loc ,[0,0,0,0]
        else:
            crop, M, com,bounds = hd.cropArea3D(com=loc, size=self.sync['config']['cube'],dsize=self.sync['config']['im_size'])
    ...
    def show3(self, frame, pose,dataset,bounds):
        """
        ...
        for i in range(jtI.shape[0]):
            jtI[i, 0]=ymin+jtI[i, 0]
            jtI[i, 1]= xmin+jtI[i, 1]

            if jtI[i, 2]>jtI[index_J_max_Depth, 2] and i in [0,2,4,6,8]:
                index_J_max_Depth = i
            cv2.circle(imgcopy, (jtI[i, 0],jtI[i, 1]), 6, (255, 255, 255), -1)

        cv2.circle(imgcopy, (jtI[index_J_max_Depth, 0],jtI[index_J_max_Depth, 1]), 12, (0, 0, 255), -1)
        cv2.putText(imgcopy, 'max depth :'+str(jtI[index_J_max_Depth, 2])+'  corresp. fingertip : '+str(index_J_max_Depth), (100, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 150, 0))
